...
const binary=resolve(process.argv[2]);const dir=mkdtempSync(join(tmpdir(),'ostrm-smoke-'));let child;
...
await api('POST','/auth/sign-up',{username:'smoke',password:'smoke-secret'});token=(await api('POST','/auth/sign-in',{username:'smoke',password:'smoke-secret'})).token;
const c=await api('POST','/openlist-config',{username:'mock',baseUrl:`http://127.0.0.1:${mock.address().port}`,token:'mock'});const t=await api('POST','/task-config',{taskName:'movies',path:'/media',openlistConfigId:c.id,isIncrement:true});await api('POST',`/task-config/${t.id}/submit`,{});
let r;for(let i=0;i<100;i++){r=await api('GET',`/task-config/${t.id}/runs/latest`);if(r&&['SUCCESS','FAILED','PARTIAL_SUCCESS'].includes(r.status))break;await new Promise(r=>setTimeout(r,100))}if(r?.status!=='SUCCESS')throw Error(JSON.stringify(r));const content=readFileSync(join(t.strmPath,'movie.strm'),'utf8');if(!content.includes('sign=test%3D%3A0'))throw Error('STRM mismatch');await stop();await start();token=(await api('POST','/auth/sign-in',{username:'smoke',password:'smoke-secret'})).token;if((await api('GET','/task-config')).length!==1)throw Error('Restart lost data');
console.log(JSON.stringify({binary,platform:process.platform,arch:process.arch,embeddedUI:true,auth:true,strm:true,restart:true}));
