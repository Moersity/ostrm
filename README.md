> **Go 原生版本（dev）**：已实现内嵌网页、跨平台单文件运行和安装包，不需要 Docker/Java。参见 [Go 使用说明](README-GO.md)、[验证报告](docs/rewrite/progress.md) 与 [Actions 构建](https://github.com/Moersity/ostrm/actions/workflows/go-ci.yml)。下方保留上游原版说明。
